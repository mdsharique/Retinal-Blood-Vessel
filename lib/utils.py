...
def recompose3D_overlap(preds, img_h, img_w, stride_h, stride_w):
    ...
    raw_pred_martrix = np.zeros((N_full_imgs, img_h, img_w))
    raw_sum = np.zeros((N_full_imgs, img_h, img_w))
    final_matrix = np.zeros((N_full_imgs, img_h, img_w),dtype='uint16')
    ...
    for i in range(N_full_imgs):
        for h in range((img_h-patch_h)//stride_h+1):
            for w in range((img_w-patch_w)//stride_w+1):
                raw_pred_martrix[i,h*stride_h:(h*stride_h)+patch_h,w*stride_w:(w*stride_w)+patch_w]+=preds[k]
                raw_sum[i,h*stride_h:(h*stride_h)+patch_h, w*stride_w:(w*stride_w)+patch_w]+=1.0
                k+=1
