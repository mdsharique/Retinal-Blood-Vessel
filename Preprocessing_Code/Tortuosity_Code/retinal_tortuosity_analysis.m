...
% path to images
UserDefined.DefaultDir = 'C:\Users\RKK-LMS\Documents\MATLAB';
image_format = 'png';
%{
...
[file_name_l, path_name_l, filt_idx_l] = uigetfile([UserDefined.DefaultDir,strcat('*.',image_format)],'Choose Segmented Image ');
...
[~,img_name_l, ext_l] = fileparts(file_name_l);
%{
mkdir(img_name)
...
ImageStruct_l = imfinfo([path_name_l [img_name_l,ext_l]]);
if numel(ImageStruct) == 1
    ImageStruct.IntImages = imread(ImageStruct.Filename,image_format);
    ImageStruct.BW = imread(ImageStruct_l.Filename, image_format);
